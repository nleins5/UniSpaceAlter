  ...
  return (
    <div className="max-w-[1600px] mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Chart */}
        <div className="lg:col-span-2 p-6 flex flex-col adm-glass">
          <div className="flex justify-between items-center mb-6 border-b border-[#333] pb-4">
            <div>
              <h3 className="text-lg font-black uppercase tracking-widest">Performance Growth</h3>
              <p className="adm-mono text-xs text-[#9ca3af] mt-1">DATA_SET: Q3_AGGREGATE // INTERVAL: H1</p>
            </div>
            <div className="flex gap-2">
              {["1D","7D","30D"].map((t,i) => (
                <button key={t} className={`adm-mono px-3 py-1 text-xs border transition-colors ${i===1 ? 'border-[#7C3AED] bg-[#7C3AED]/10 text-[#7C3AED]' : 'border-[#333] text-[#9ca3af] hover:bg-[#333] hover:text-white'}`}>{t}</button>
              ))}
            </div>
          </div>
          <div className="flex-1 relative min-h-[300px] border-l border-b border-[#333] ml-10">
            {/* Y axis */}
            <div className="adm-mono absolute left-[-40px] top-0 bottom-0 flex flex-col justify-between text-[10px] text-[#9ca3af] text-right w-8">
            ...
            {/* Tooltip */}
            <div className="adm-mono absolute left-[80%] top-[35%] -translate-x-1/2 -translate-y-full mb-2 bg-[#1a1a1a] border border-[#7C3AED]/50 p-2 text-xs shadow-lg">
              <div className="text-[#7C3AED] mb-1">VAL: 84.2K</div>
              <div className="text-[#9ca3af]">TME: 14:00</div>
            </div>
            {/* X axis */}
            <div className="adm-mono absolute bottom-[-24px] left-0 right-0 flex justify-between text-[10px] text-[#9ca3af] px-4">
        ...
        {/* System Logs */}
        <div className="p-6 flex flex-col adm-glass">
          <h3 className="text-lg font-black uppercase tracking-widest border-b border-[#333] pb-4 mb-4">System Logs</h3>
          <div className="adm-mono flex-1 overflow-hidden space-y-3 text-xs">
            {LOGS.map((l,i) => (
          ...
          <div className="mt-4 pt-4 border-t border-[#333] flex justify-between items-center">
            <span className="adm-mono text-[10px] text-[#9ca3af]">STATUS: LISTENING...</span>
